...
class DrugDataLoader(object):
    ...
    def _generate_topoy_graph(self):
        ...
        for cv in range(0, 10):
            ...
            self.train_enc_graph = self._generate_enc_graph(train_pairs, train_values,
                                                            add_support=True)
            self.subgraphs = self._generate_hetero_subgraphs(self.train_enc_graph, num_subgraphs=3)
            self.train_dec_graph = self._generate_dec_graph(train_pairs)
            self.train_truths = th.FloatTensor(train_values)
    ...
    def _generate_hetero_subgraphs(self, enc_graph, num_subgraphs=3):
        """
        生成多个异构子图，所有子图包含所有药物和疾病节点，。
        ...
        返回:
            subgraphs: 包含多个子图的列表，每个子图包含所有节点但边不完全相同。
        """
        # 获取药物和疾病节点的数量
        num_drugs = enc_graph.number_of_nodes('drug')
        num_diseases = enc_graph.number_of_nodes('disease')
        ...
        # 获取所有边的信息
        edge_dict = {}
        for etype in enc_graph.etypes:
            # 获取每种边的源节点和目标节点
            edge_dict[etype] = enc_graph.edges(etype=etype)

        # 计算每个子图分配的边数
        for etype in edge_dict:
            num_edges = len(edge_dict[etype][0])
            edges_per_subgraph = num_edges // num_subgraphs  # 每个子图的边数

            # 随机打乱边
            perm = th.randperm(num_edges)
            edge_dict[etype] = (edge_dict[etype][0][perm], edge_dict[etype][1][perm])
        ...
        for i in range(num_subgraphs):
            ...
            for etype in edge_dict:
                ...
                if i == num_subgraphs - 1:
                    # 最后一个子图分配剩余的所有边
                    end_idx = len(edge_dict[etype][0])
                else:
                    end_idx = (i + 1) * edges_per_subgraph

                # 获取当前子图的边
                sub_edges_src = edge_dict[etype][0][start_idx:end_idx]
                sub_edges_dst = edge_dict[etype][1][start_idx:end_idx]

                # 从 etype 获取 src_type, edge_type 和 dst_type
                src_type, edge_type, dst_type = enc_graph.to_canonical_etype(etype)

                # 添加边到子图的data_dict，保持三元组格式
                data_dict[(src_type, edge_type, dst_type)] = (sub_edges_src, sub_edges_dst)

            # 创建子图，确保子图包含所有的药物和疾病节点
            subgraph = dgl.heterograph(data_dict, num_nodes_dict={'drug': num_drugs, 'disease': num_diseases})

            # 复制节点特征（ci, cj）到子图
            for ntype in ['drug', 'disease']:
                for feat_name in ['ci', 'cj']:
                    if feat_name in enc_graph.nodes[ntype].data:
                        subgraph.nodes[ntype].data[feat_name] = enc_graph.nodes[ntype].data[feat_name]
